...
export const getLuxColor = (lux: number, roadType: RoadType = RoadType.RESIDENTIAL): string => {
  ...
  switch (roadType) {
    case RoadType.MAIN_ROAD:
      ...
      if (lux < 10) return colors.deepRed;    // <10 - Deep Red
      if (lux < 15) return colors.red;        // 10-15
      ...
      if (lux < 27) return colors.lightGreen; // 25-27
      if (lux < 29) return colors.mediumGreen; // 27-29
      if (lux < 30) return colors.green;      // 29-30
      return colors.deepGreen;                // >30
      
    case RoadType.HIGHWAY:
      // Highways: Green starts at >50 lux, red is <20 lux
      if (lux < 20) return colors.deepRed;    // <20 - Deep Red
      if (lux < 25) return colors.red;        // 20-25
      ...
      if (lux < 45) return colors.lightGreen; // 42-45
      if (lux < 48) return colors.mediumGreen; // 45-48
      if (lux < 50) return colors.green;      // 48-50
      return colors.deepGreen;                // >50
      
    case RoadType.RESIDENTIAL:
    default:
      // Residential roads: Light green starts from 20 lux
      if (lux < 5) return colors.deepRed;     // <5 - Deep Red
      if (lux < 8) return colors.red;         // 5-8
      ...
      if (lux < 18) return colors.lightGreen; // 16-18
      if (lux < 20) return colors.mediumGreen; // 18-20
      if (lux < 22) return colors.green;      // 20-22
