...
function Analytics() {
  ...
  const getSortedBarangays = () => {
    const filteredBarangays = Object.keys(barangayGroups)
    ...
    return filteredBarangays.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'wellLit': {
          const percentA = dataA.length > 0 
            ? Math.round((dataA.filter(s => s.lux >= WELL_LIT_THRESHOLD).length / dataA.length) * 100) 
            : 0;
          const percentB = dataB.length > 0 
            ? Math.round((dataB.filter(s => s.lux >= WELL_LIT_THRESHOLD).length / dataB.length) * 100) 
            : 0;
          return sortDirection === 'asc' 
            ? percentA - percentB 
            : percentB - percentA;
        ...
        case 'avgLux': {
          const avgA = dataA.length > 0
            ? Math.round(dataA.reduce((sum, s) => sum + s.lux, 0) / dataA.length)
            : 0;
          const avgB = dataB.length > 0
            ? Math.round(dataB.reduce((sum, s) => sum + s.lux, 0) / dataB.length)
            : 0;
          return sortDirection === 'asc' 
            ? avgA - avgB 
            : avgB - avgA;
        ...
        <div>
          ...
          <div className="flex justify-between items-center mb-2">
            <div className="text-sm text-gray-400">
              Showing {getSortedBarangays().length} of {Object.keys(barangayGroups).length} barangays
            </div>
            {sortField !== 'name' && (
              <button 
              ...
              <table className="min-w-full divide-y divide-gray-700">
                <thead>
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Main Street</th>
                  ...
                  {getSortedBarangays().map((name, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex items-center">
                            <span className={`text-sm font-medium ${wellLitPercentage < 50 ? 'text-amber-400' : 'text-emerald-400'}`}>
                              {wellLitPercentage}%
                            </span>
                            <span className="text-xs text-gray-400 ml-2">({wellLitSensors}/{totalSensors})</span>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`text-sm font-medium ${avgLux < WELL_LIT_THRESHOLD ? 'text-amber-400' : 'text-emerald-400'}`}>
                            {avgLux} lux
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className="text-xs text-gray-400">
                            {latestTimestamp ? latestTimestamp.toLocaleDateString() : 'No data'}
                          </span>
                        </td>
