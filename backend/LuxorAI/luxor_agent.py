...
class LuxorAgent:
    ...
    def __init__(
        ...
        if ollama is not None:
            try:
                ...
                # Try to show the model; ignore errors here so agent still starts
                try:
    ...
    def answer(
        ...
        *,
        ...
        """Main entrypoint: return answer, context and metadata; robust to errors."""
        try:
            if self._is_greeting(question):
                return self._build_greeting_response(question)

            context = self.gather_context(question, limit=limit)
            context = context or {"illumination": [], "barangays": [], "streets": []}
            ...
            def _matches_list_pattern(text: str, target: str) -> bool:
                patterns = [
                    rf"list\s+(all\s+)?{target}",
                    rf"list\s+of\s+(all\s+)?{target}",
                    rf"all\s+{target}",
                    rf"what\s+{target}",
                    rf"which\s+{target}",
                    rf"show\s+{target}",
                    rf"{target}\s+in\s+dataset",
                    rf"{target}\s+list",
                    rf"available\s+{target}",
                ]
            ...
            if _matches_list_pattern(qnorm, "barangay") and barangay_names:
                ...
                data_source = "Structured dataset only"
            elif ("how many barangay" in qnorm or "number of barangay" in qnorm or "count barangay" in qnorm) and barangay_names:
                answer_text = f"There are {len(barangay_names)} barangays in your dataset."
                data_source = "Structured dataset only"
            elif ("how many street" in qnorm or "number of street" in qnorm or "count street" in qnorm) and street_names:
                answer_text = f"There are {len(street_names)} streets in your dataset."
                data_source = "Structured dataset only"
            else:
                if has_data:
                    try:
                        ...
                        answer_text = (
                            "An error occurred while summarizing structured data. "
                            "Please try a simpler query or check your dataset for missing fields."
                        )
                        data_source = f"Error summarizing data: {exc}"
            ...
            print(f"LuxorAgent.answer error: {exc}")
            import traceback
            traceback.print_exc()
            safe_context = self._make_json_safe({"illumination": [], "barangays": [], "streets": []})
            return {
                "question": question,
                "answer": "❌ Internal error while processing the request. Please try again or contact the administrator.",
                "context": safe_context,
                "model": None,
                "used_llm": False,
    ...
    def _is_general_knowledge_question(self, question_lower: str) -> bool:
        ...
        if (any(kw in question_lower for kw in balanga_keywords) and any(kw in question_lower for kw in general_keywords)):
            return True
    ...
    def _validate_and_correct_response(self, question: str, response: str) -> str:
        ...
        for wrong, correct in corrections.items():
            if wrong in response_lower:
                corrected_response = re.sub(re.escape(wrong), correct, corrected_response, flags=re.IGNORECASE)
    ...
    def _is_greeting(question: str) -> bool:
        ...
        domain_tokens = {
            ...
            "sensor", "sensors", "reading", "readings", "light", "lights",
        }
        if len(tokens) <= 4 and any(token in greeting_tokens for token in tokens):
            if not any(token in domain_tokens for token in tokens):
                return True
        phrases = {"good morning", "good afternoon", "good evening", "good day", "thank you"}
    ...
    def _format_structured_summary(context: Dict[str, List[Dict]]) -> str:
        ...
        barangay_names = {rec.get("id"): rec.get("name") for rec in barangays if rec.get("id") is not None and rec.get("name")}
        ...
        if barangays:
            primary_barangay = list(barangay_names.values())[0] if barangay_names else (barangays[0].get("name") if barangays and barangays[0].get("name") else "Unknown")
    ...
    def _make_json_safe(value: Any) -> Any:
        ...
        # Local import to avoid hard dependency when pandas isn't installed
        try:
        ...
        if isinstance(value, dict):
            return {str(k): LuxorAgent._make_json_safe(v) for k, v in value.items()}

        if isinstance(value, (list, tuple, set)):
            return [LuxorAgent._make_json_safe(v) for v in value]

        if isinstance(value, (datetime, date)):
            return value.isoformat()

        if Timestamp is not None and isinstance(value, Timestamp):  # type: ignore
            return value.isoformat()
