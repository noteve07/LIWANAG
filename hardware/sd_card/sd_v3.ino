...
bool acquireInitialGPSFix(unsigned long timeoutMs) {
	...
	logMessage("GPS initial fix timed out");
	return false;
}
...
void handleButtons() {
	...
	if (startState == LOW && lastStartButtonState == HIGH) {
		if (!gpsReady) {
			logMessage("Waiting for GPS fix before start");
		} else if (!collecting) {
			collecting = true;
...
void setup() {
	...
	while (!acquireInitialGPSFix(gpsTimeout)) {
		logMessage("GPS lock unavailable, retrying...");
	}
	...
	readyBlinkState = false;
	digitalWrite(LED_BUILTIN, LOW);
	logMessage("GPS lock acquired - waiting for START button");
